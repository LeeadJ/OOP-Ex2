...
public class DWGalgo implements DirectedWeightedGraphAlgorithms {
    ...
    private double warshall(int src, int dest, double[][] node_matrix) {
        ...
        System.out.println(node_matrix[src][dest]);
        System.out.println(Arrays.deepToString(node_matrix));
    ...
    public List<NodeData> shortestPath(int src, int dest) {
        ...
        return warshallNodePath(src, dest, node_matrix, nodePath);
    }
     ...
     * Return: Double.
     */
    private List<NodeData> warshallNodePath(int src, int dest, double[][] node_matrix, List<NodeData> list) {
        List<NodeData> final_list = new ArrayList<>();
        int len = node_matrix.length;
        for (int k = 0; k < len; k++)
            for (int i = 0; i < len; i++)
                for (int j = 0; j < len; j++) {
                    ...
                    if (node_matrix[i][k] > 0.0 && node_matrix[k][j] > 0.0) {
                        if (node_matrix[i][j] == 0.0 && i != j) {
                            node_matrix[i][j] = node_matrix[i][k] + node_matrix[k][j];
                            if (i == src || j == dest) {
                                list.add(this.graph.getNode(k));
                                list.add(this.graph.getNode(j));
                            }
                            continue;
                        }
                        if (node_matrix[i][k] + node_matrix[k][j] < node_matrix[i][j]) {
...
//            }
//        else{
//            list.add(0,this.graph.getNode(src));
//        }
        System.out.println(Arrays.deepToString(node_matrix));
        for (NodeData gn : list) {
            if (!final_list.contains(gn)) {
                final_list.add(gn);
            }
        }
        final_list.add(0, this.graph.getNode(src));
        return final_list;
    }
